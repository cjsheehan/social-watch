...
import { tweetStats, addSentiment, tokenise, timeBins } from "./twitterStats"
...
describe("twitterStats", function () {
	...
	describe("timebins", function () {
		...
		it("time bins", function () {
			chai.assert.equal(actual[0].start.toString(), moment("Fri Jul 29 18:27:18 +0000 2016", [CAT_FORMAT], "en").toString());

			chai.assert.equal(actual[0].end.toString(), moment("Fri Jul 29 17:27:18 +0000 2016", [CAT_FORMAT], "en").toString());

			chai.assert.equal(actual[1].start.toString(), moment("Fri Jul 29 17:27:18 +0000 2016", [CAT_FORMAT], "en").toString());

			chai.assert.equal(actual[1].end.toString(), moment("Fri Jul 29 16:27:18 +0000 2016", [CAT_FORMAT], "en").toString());
		});
